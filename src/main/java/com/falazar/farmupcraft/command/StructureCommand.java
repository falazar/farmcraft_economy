...
/**
 * StructureCommand provides commands for finding and managing structures in the world.
 * It includes commands to find nearby structures and list structures from the database.
 */
public class StructureCommand {
        ...
        LiteralArgumentBuilder<CommandSourceStack> findBuilder = Commands.literal("find")
                ...
                .then(Commands.argument("filter", StringArgumentType.string())
                        ...
                        .executes(context -> {
                            ...
                            int defaultRadius = 5;
                            findNearbyStructures(context.getSource(), filter, defaultRadius); // Pass the filter to the method with default radius
                            return 0;
                        })
                        .then(Commands.argument("radius", IntegerArgumentType.integer(1, 50))
                                .executes(context -> {
                                    ...
                                    findNearbyStructures(context.getSource(), filter, radius); // Pass both filter and radius
                                    return 0;
                                })
                        )
                );
    ...
    public static int findNearbyStructures(CommandSourceStack source, String filter, int radius) {
        ...
        List<StructureInfo> sortedStructures = findNearbyStructuresForVillage(BlockPos.containing(source.getPosition()), world, playerSource, playerVillage, filter, radius);
        ...
        for (final StructureInfo structureInfo : sortedStructures) {
            ...
            MutableComponent baseMessage = Component.literal(
                    (sortedStructures.indexOf(structureInfo) + 1) + "." // index starts at 0, so add 1
                                    + " " + getCommonNameForStructure(structureInfo.getType()) + claimedStatus + visitedStatus);  // common name
    ...
    public static List<StructureInfo> findNearbyStructuresForVillage(BlockPos centerPos, ServerLevel world, Player playerSource, VillageData village, String filter, int radius) {
        ...
        for (Map.Entry<Structure, LongSet> structureEntry : structures.entrySet()) {
            String type = world.registryAccess().registry(Registries.STRUCTURE).get().getKey(structureEntry.getKey()).toString();
            ...
            for (Long id : structureEntry.getValue()) {
                ...
                world.structureManager().fillStartsForStructure(structureEntry.getKey(), singleIdSet,
                        structureStart -> processStructureStart(structureStart, type, id, structureInfos, gameStructureDatabase, newStructuresSaved, filter, structureEntry));
            }
        }
        ...
        LOGGER.info("Found " + structureInfos.size() + " structures, saved " + newStructuresSaved[0] + " NEW structures to GameStructureData database");
        
        return structureInfos;
    }

    /**
     * Processes a single structure start and adds it to the database and results list.
     * 
     * This method handles the complex logic for processing individual structure instances found in the world.
     * It performs several key operations:
     * 
     * 1. **Position Calculation**: Determines the center position of the structure, with special handling
     *    for structures that have incorrect Y coordinates (like mineshafts and dungeons)
     * 
     * 2. **Filtering**: Applies height-based filtering based on the provided filter parameter
     ...
     * 
     * 3. **Database Storage**: Saves the structure to the GameStructureData database if it doesn't
     *    already exist, using the structure's unique Long ID as the key
     * 
     * 4. **Results Collection**: Adds the structure to the results list for display to the user
     * 
     * 
     * @param structureStart The Minecraft structure start object containing bounding box and other data
     * @param type The structure type identifier (e.g., "mineshaft", "dungeon")
     * @param id The unique Long ID for this specific structure instance
     * @param structureInfos The list to add the processed structure to for display
     * @param gameStructureDatabase The database to store the structure data in
     * @param newStructuresSaved Counter array to track how many new structures were saved
     * @param filter The height filter to apply ("upper", "lower", or "all")
     * @param structureEntry The original structure entry containing the LongSet for logging
     */
    private static void processStructureStart(StructureStart structureStart, 
        ...
        // If the y min value is less than 50 and the y max is greater than 10, manually set the y value to y min + 50 only!
        if (structureStart.getBoundingBox().minY() <= 60 && structureStart.getBoundingBox().maxY() > 100) {
            LOGGER.info("HACKING structure at Y=" + structureStart.getBoundingBox().minY() + " because it is below 60.");
        ...
        if (!gameStructureDatabase.containsKey(id)) {
            ...
            LOGGER.info("Saved NEW structure to database: " + type + " (ID: " + id + ") at " + structureCenterPos + " claimed: " + isOnClaimedPlot);
        } else {
            ...
            if (existingData != null) {
                ...
                if (existingData.isOnClaimedPlot() != isOnClaimedPlot) {
                    ...
                    gameStructureDatabase.setDirty();
                    LOGGER.info("Updated structure claimed status: " + type + " (ID: " + id + ") claimed: " + isOnClaimedPlot);
                }
            }
    ...
    public static int listStructuresFromDatabase(CommandSourceStack source, int count, String filter) {
        ...
        if (gameStructureDatabase.getSize() == 0) {
            source.sendSystemMessage(Component.literal("No structures found in database.").withStyle(ChatFormatting.YELLOW));
            return 0;
        }
            ...
            if (structureData != null) {
                ...
                switch (filter.toLowerCase()) {
                    ...
                    case "village":
                        ...
                        break;
                    default:
                        source.sendFailure(Component.literal("Invalid filter: " + filter + ". Use: all, claimed, unclaimed, visited, unvisited, village"));
                        return 0;
                }
        ...
        if (structuresList.isEmpty()) {
            source.sendSystemMessage(Component.literal("No structures found matching filter: " + filter).withStyle(ChatFormatting.YELLOW));
            return 0;
        }
        ...
        source.sendSystemMessage(Component.literal("Structures in database (showing first " + count + " by distance, filter: " + filter + "):").withStyle(ChatFormatting.GOLD));
        ...
        for (Map.Entry<Long, GameStructureData> entry : structuresList) {
            ...
            String status = "";
            if (structureData.isOnClaimedPlot()) status += " [CLAIMED]";
            if (structureData.wasVisited()) status += " [VISITED]";
            
            // Check if structure is out of range (no chunk data means far from village areas)
            ...
            LOGGER.info("StructureData: " + structureData.getName() + " (" + structureData.getType() + ") at " + structureData.getCenterPos().toShortString() + " ID: " + structureId + " d=" + distance + status);
     ...
     * This should be called whenever a chunk gets claimed or unclaimed by a village.
     * 
    ...
    public static void updateStructuresInChunk(ChunkPos chunkPos, boolean isClaimed, ServerLevel world) {
        try {
            ...
            for (Long structureId : gameStructureDatabase.getKeys()) {
                ...
                // Update claimed status if it changed
                if (structureData.isOnClaimedPlot() != isClaimed) {
    ...
    public static int unmarkStructureAsVisited(CommandSourceStack source, long structureId) {
        try {
            ...
            if (!structureData.wasVisited()) {
                source.sendFailure(Component.literal("Structure " + structureData.getName() + " (ID: " + structureId + ") is not marked as visited."));
                return 0;
            }
            ...
            source.sendSystemMessage(Component.literal("Unmarked structure as visited: " + structureData.getName() + " (ID: " + structureId + ")").withStyle(ChatFormatting.GREEN));
    ...
    // TODO make a village method now that locates all structures near a village area.
    // then logs them to the village, and to the DB.
    // then we need to know if they have been visited somehow, by a player or a village, and save that.
...
[07:55:14]   [TesterCommand:172] key = com.yungnickyoung.minecraft.bettermineshafts.world.BetterMineshaftStructure@37e152ff longset={-412316860221, -390842023739}
[07:55:14]   [TesterCommand:174] Bounds = BoundingBox{minX=3085, minY=-26, minZ=-1537, maxX=3214, maxY=320, maxZ=-1429}

[10:52:56] [Server thread/INFO] [co.fa.fa.ut.CustomLogger/]: [INFO] [farmupcraft] [StructureCommand:273] Type = valhelsia_structures:deep_spawner_room
10:52:56.540
game
[10:52:56] [Server thread/INFO] [co.fa.fa.ut.CustomLogger/]: [INFO] [farmupcraft] [StructureCommand:274] key = 
com.stal111.valhelsia_structures.common.world.structures.ValhelsiaJigsawStructure@42137192 
...
[17:17:46]   [TesterCommand:292] key = com.yungnickyoung.minecraft.bettermineshafts.world.BetterMineshaftStructure@65223073 longset={270582939573}
[17:17:46]   [TesterCommand:294] Bounds = BoundingBox{minX=-1249, minY=-45, minZ=866, maxX=-1138, maxY=320, maxZ=1019}

[17:17:46]   [TesterCommand:291] Type = dungeoncrawl:dungeon
[17:17:46]   [TesterCommand:292] key = xiroc.dungeoncrawl.dungeon.Dungeon@56327917 longset={219043332008}
[17:17:46]   [TesterCommand:294] Bounds = BoundingBox{minX=-1470, minY=-7, minZ=720, maxX=-1328, maxY=512, maxZ=878}
...
[17:17:46] [CHAT] 1. towns_and_towers:village_birch_forest a. -1302, 74, 916 d=17
[17:17:46] [CHAT] 9. bettermineshafts:mineshaft_lush a. -1193, 138, 943 d=146  BAD
