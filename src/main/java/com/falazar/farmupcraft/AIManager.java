...
public class AIManager {
    ...
    private static final String SETTING_CONTEXT =
            "You are a character designer for a cozy medieval Minecraft village mod. ";

    /** Shared paragraph structure instructions used in every personality prompt. */
    private static final String PARAGRAPH_INSTRUCTIONS =
    ...
    private static String buildVillagerNamesHint(List<String> villagerNames) {
        if (villagerNames == null || villagerNames.isEmpty()) return "";
        return "Other villagers in the village: " + String.join(", ", villagerNames) + ".\n";
    }

    /** Formats village biomes into a context hint. Returns empty string if null/empty. */
    ...
    /**
     * Builds an optional sentence making the villager curious about one nearby structure.
    ...
    private static String buildStructureCuriosityHint(String structureName) {
        if (structureName == null || structureName.isBlank()) return "";
     ...
     * @param structureName    one nearby structure to make the villager curious about (may be null)
    ...
    public static CompletableFuture<String> generateNpcPersonality(String name, String profession, List<String> villagerNames) {
     ...
     * @param existingProfile the full text of the existing JSON profile (may be null/empty)
     * @param villagerNames   names of other villagers in the village (may be null)
     * @param villageName     name of the village (may be null)
     * @param biomes          biome names present in the village (may be null)
     * @param structureName   one nearby structure to make the villager curious about (may be null)
